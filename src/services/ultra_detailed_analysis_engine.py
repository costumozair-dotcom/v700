...
class UltraDetailedAnalysisEngine:
    ...
    def _execute_anti_objection(self, avatar_data: Dict[str, Any], data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        import asyncio
        try:
            anti_objection_instance = AntiObjectionSystem()
            anti_objection_result = asyncio.run(anti_objection_instance.criar_sistema_anti_objecao({
                'avatar': avatar_data,
                'produto': data.get('produto', 'Produto'),
                'funil_vendas': data.get('funil_vendas', {}),
                'pesquisas': data.get('pesquisas', []),
                'historico_objecoes': objections
            }))
        except Exception as e:
            logger.error(f"Erro ao executar sistema anti-objeção: {e}")
            anti_objection_result = None
